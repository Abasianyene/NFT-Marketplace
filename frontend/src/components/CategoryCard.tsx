...
const CategoryCard: React.FC<CategoryCardProps> = ({ img, icon, label }) => {
  return (
    <div className="bg-surface rounded-[20px] group cursor-pointer">
      <div className="relative rounded-t-[20px] grid place-items-center overflow-hidden h-[250px]">
        ...
        <img src={img} alt={label} className="w-full h-full object-cover filter transition-all duration-300 blur-sm group-hover:blur-lg" />
      ...
      <div className="p-3">
        <h2 className="text-main font-bold text-xl">{label}</h2>
      </div>
    </div>
  )
}
