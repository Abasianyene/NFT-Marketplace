interface CreatorCardProps {
  rank: number;
  img: string; // <-- changed from image to img
  name: string;
  sales: string;
}


const CreatorCard: React.FC<CreatorCardProps> = ({ rank, img, name, sales }) => {
  return (
  <div className="bg-surface group border shadow-lg border-transparent rounded-[10px] p-4 relative grid place-items-center transition-all duration-300 hover:shadow-xl group cursor-pointer hover:bg-background hover:border-surface">
      ...
      <img src={img} alt={name} className="rounded-full w-[150px] h-[150px] transition-transform duration-300 group-hover:-translate-y-2" />
